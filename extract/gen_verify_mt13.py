"""
Generate data/games51/floors/MT13.json from mt13_raw_capture.json.
...
def build_entities_annotation(floor_map, tiles_db):
    ...
    ALTAR_DECOR_IDS = {"blueShop-left","blueShop-right"}
...
# ── Load ─────────────────────────────────────────────────────────────────────
raw      = load_raw_capture(EXTRACT / "mt13_raw_capture.json")
tiles_db = json.loads((DATA / "tiles.json").read_text(encoding="utf-8"))
entities = build_entities_annotation(raw["map"], tiles_db)

# ── Build output ──────────────────────────────────────────────────────────────
out = {
    "_comment":      "来源: core.floors['MT13'] (live engine, Playwright 提取)。events 存原始 h5mota 脚本。",
    "floorId":       raw["floorId"],
    "title":         raw["title"],
    "width":         raw["width"],
    "height":        raw["height"],
...
# ── Write ─────────────────────────────────────────────────────────────────────
out_path = DATA / "floors" / "MT13.json"
...
print(f"  firstArrive 条目数: {len(raw.get('firstArrive', []))}")
print(f"  afterBattle 条目数: {len(raw.get('afterBattle', {}))}")
print(f"  autoEvent 条目数: {len(raw.get('autoEvent', {}))}")
for cat, tiles in ents.items():
    print(f"  {cat}: {list(tiles.keys())}")
has_altar = '6,9' in raw.get('events', {}) or any(
    v == 131 for row in raw['map'] for v in row
)
print(f"\n  祭坛(blueShop tile=131): {'存在' if any(v==131 for row in raw['map'] for v in row) else '不存在'}")
altar_locs = [(x,y) for y,row in enumerate(raw['map']) for x,v in enumerate(row) if v==131]
if altar_locs:
    print(f"  祭坛坐标: {altar_locs}")
    for loc in altar_locs:
        key = f"{loc[0]},{loc[1]}"
        if key in raw.get('events', {}):
            print(f"  events['{key}'] 存在 ✓")
