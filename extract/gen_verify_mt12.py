"""
Generate data/games51/floors/MT12.json from mt12_raw_capture.json.
Cell-by-cell verification against the raw engine capture is the authoritative check.
"""
import json
import re
        ...
        for x, cell in enumerate(row):
            ...
            if cell in enemy_map:
                ...
                if tid in STAIR_IDS:
                    ...
                    npcs.setdefault(key, []).append(pos)   # altar decor grouped with NPCs
                else:
...
# ── Load ────────────────────────────────────────────────────────────────────
raw      = load_raw_capture(EXTRACT / "mt12_raw_capture.json")
tiles_db = json.loads((DATA / "tiles.json").read_text(encoding="utf-8"))
entities = build_entities_annotation(raw["map"], tiles_db)

# ── Build output ─────────────────────────────────────────────────────────────
out = {
    "_comment":      "来源: core.floors['MT12'] (live engine, Playwright 提取)。events 存原始 h5mota 脚本。",
    "floorId":       raw["floorId"],
    "title":         raw["title"],
    "width":         raw["width"],
    "height":        raw["height"],
...
# ── Write ────────────────────────────────────────────────────────────────────
out_path = DATA / "floors" / "MT12.json"
...
print(f"\n  祭坛确认: events['6,9']={'6,9' in raw.get('events',{})}")
