"""
Generate data/games51/floors/MT17.json from mt17_raw_capture.json.
...
raw      = load_raw_capture(EXTRACT / "mt17_raw_capture.json")
tiles_db = json.loads((DATA / "tiles.json").read_text(encoding="utf-8"))
entities = build_entities_annotation(raw["map"], tiles_db)

out = {
    "_comment":      "来源: core.floors['MT17'] (live engine, Playwright 提取)。events 存原始 h5mota 脚本。",
    "floorId":       raw["floorId"],
    "title":         raw["title"],
    "width":         raw["width"],
    "height":        raw["height"],
...
out_path = DATA / "floors" / "MT17.json"
...
print(f"  岩浆(tile 5): {'存在，位置: '+str(lava_locs) if lava_locs else '不存在'}")
for tid, tname in [(81,'yellowDoor'),(82,'blueDoor'),(83,'redDoor'),(85,'specialDoor')]:
    locs = [(x,y) for y,row in enumerate(raw['map']) for x,v in enumerate(row) if v==tid]
    if locs:
        print(f"  {tname}(tile {tid}): {locs}")

print(f"\n  afterBattle 详情:")
for k, v in raw.get('afterBattle', {}).items():
