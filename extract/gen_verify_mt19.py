"""Generate data/games51/floors/MT19.json from mt19_raw_capture.json."""
import json, re
...
out_path = DATA / "floors" / "MT19.json"
...
print(f"\n=== MT19 ===  ratio={raw['ratio']}  downFloor={raw['downFloor']}  upFloor={raw['upFloor']}")
print(f"  changeFloor: {list(raw.get('changeFloor',{}).keys())}")
print(f"  events:{len(raw.get('events',{}))}  afterBattle:{len(raw.get('afterBattle',{}))}  afterOpenDoor:{list(raw.get('afterOpenDoor',{}).keys())}")
