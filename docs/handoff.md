...
### 工作区状态（均未提交）
- `sim/simulator.py` 三段改动：move/keep-move 无 loc 修法（上会话）+ 本会话 hide 抑制 + `_suppressed_events` 注释。
- 本会话验证产物：`verify_thief19_fix_ab.py`、`probe_hide_suppress_blastradius.py`、`verify_hide_suppress_fix_emulation.py`、`diag_thief37_refire_contradiction.py`、`gen_thief19_test_route.py`、`thief19_mt2_test.h5route`。
