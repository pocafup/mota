...
#### (a) Token 编号与真值对齐：正确，无偏移

`parse_rle_route()` 生成的 token 序列中，tok[0]..tok[2199] 不存在任何字符丢弃。  
13个检查点全部在 tok[100]..tok[1300] 范围内，均属于无偏移区间。  
玩家提供的真值 token 号与我们解析出的 token 序列**一一对应，无需修正**。
...
这是真实偏差，根因尚未完全查明，候选原因：

1. **ATK−1**：真实攻击宝石（redGem）来源未被模拟器拾取。MT5(3,1) 的错误 redGem 已修正（原是提取错误），但真实 ATK+1 来源未定位。可能在 tok[201] 附近（玩家提示"token201 攻击宝石"）。
2. **yellowKey−1**：英雄在 tok[243-247] 区间应拾取 1 枚黄钥匙，模拟器漏拿。可能是路径分歧（墙 tile 误判或实体缺失导致英雄走不到该格）。
3. **HP−122**：MT4 地图修正后（(4,9) redPotion→bat，(4,11) floor→greenSlime），模拟器战斗损血与真值偏差加大。可能是战斗公式与怪物参数的局部不匹配。

**下一步**：在 tok[200..300] 区间做精细轨迹对比，定位 ATK 和 yellowKey 的具体漏拿格。
