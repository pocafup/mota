...
## ✅ MT1–MT20 全程逐 token 对齐（2026-06-03，17/17 检查点全 PASS）

扩层第一批 MT15–MT20 已彻底对齐：**tok[2000] 转绿，MT1–MT20 全程逐 token 忠实复现，54/54 测试全绿**（commit b11f9c2）。进度详见 §一、§八(0)。
...
**下一步：扩层第二批 MT21–MT28（楼梯路过层）**——提取基本地图 + 楼梯连通，端点验证 tok[2400]。

复现脚本：`diag_1730_2000.py`（逐 token 打印 tok[1730..2000] + 锚点对比；注：脚本内 tok[1680] 锚点坐标 (1,11) 系手记写反，sim 实为 (11,1)、属性全对，非检查点、不影响结论）。
