...
> 注：本节的 13 点指 tok[100..1300]，仍全 PASS。扩层第一批新增 4 点（tok[1400/1500/1603/2000]）当前 FAIL，根因与待办见 §八(0)。
...
### (0) ⭐ 扩层第一批对齐：token1400 FAIL，待修两个 bug

**背景**：已提取 MT15–MT20、实现 cross 机制，正在用扩层检查点（tok[1400/1500/1603/2000]）对齐英雄走到 MT20 这段。当前 4 个新检查点全 FAIL，根因锁定在 tok[1300]→tok[1400] 这段，**偏差 485 HP**（tok[1400] 真值 545，sim 跑出 60）。

**检查点真值（金标准，tok[n]=执行完第 n 步）**：

```
tok[1300]: MT14         785/42/30   ← 双方一致（此前 13 点全 PASS）
tok[1321]: MT11         795/42/30
tok[1341]: MT15(7,11)   705/42/30   ← 进 MT15
tok[1366]: MT15(11,2)   545/42/30   ← MT15 内损血到 545（真实损 250）
tok[1400]: MT6          545/42/30   ← 之后稳定 545
```

**sim 逐笔 HP 变化清单（tok[1300..1400]，共 5 笔战斗，无拾取/购买回血）**：

```
 tok  楼层    坐标       HP变化   打的怪
 1303  MT14  ( 8, 8)    -90    大史来姆(blackSlime)
 1326  MT11  ( 4, 9)    -90    大史来姆(blackSlime)
 1356  MT15  ( 9, 4)    -70    大蝙蝠(bigBat)
 1365  MT15  (11, 2)    -90    大史来姆(blackSlime)
 1374  MT16  ( 5, 3)   -385    兽人(zombie)   ← Bug B 多打的
```

sim 累计：785→695→605→535→445→**60**。

**两个 bug（玩家逐笔核对确认，仅此两个，无第三个）**：

- **Bug A（+100 漏拿）**：MT14(5,10) 的 redPotion（ratio×2 = +100 HP）漏拿——英雄在 MT14 没先去 (5,10) 拿药就直接下楼。导致 tok[1321] sim=695 vs 真值 795。
- **Bug B（-385 多损）**：英雄进 MT16 后**走偏到 (5,3)**，打了真实路线**没打**的兽人（zombie，-385 HP）。真实游戏 tok[1366]→tok[1400] HP 稳定 545，说明这只兽人这趟没打。

  ⚠ **MT16 落点 (6,2) 是对的**（玩家确认），不是落点错。问题是落点 (6,2) 之后英雄走偏到 (5,3)。修复方向：查 MT16 内 tok[1366..1374] 英雄为何走到 (5,3)，对比真实路线该走哪。**不要去改落点约定。**

**MT15 段已排除**：sim 在 MT15 正确触发小偷(9,1)开墙(8,1)、正确绕过 steelRock(DEF=315)，MT15 内损血 -160（bigBat -70 + blackSlime -90）与真实一致，MT15 不是问题源。

**下个会话先修这两个 bug，再验证 tok[1500/1603/2000]。先别动其他。**
