...
> 注：本节的 13 点指 tok[100..1300]，仍全 PASS。扩层新增 4 点中 tok[1400/1500/1603] 已转绿（commit e7f20d0、75687ea），仅 tok[2000] 仍 FAIL；现状与待办见 §八(0)。合计 16/17 PASS。
...
### (0) ⭐ 扩层对齐进度（2026-06-03 更新）：tok[1400/1500/1603] 已转绿，tok[2000] 仍 FAIL

**已解决（16/17 检查点 PASS，零回归）**：

- tok[1400]（commit e7f20d0）：祭坛格 7/8 标 noPass + 模拟器数据驱动读 noPass（不再硬编码 WALL_TILES）+ 互动格统一不移入（fakeWall/祭坛/激活 NPC）。
- tok[1500] / tok[1603]（commit 75687ea）：区分商人(trader 122)与祭坛(blueShop 131)两套独立脚本，实现商人交易（卖钥匙→二次对话消失→全程不移入），连锁转绿。

**仍 FAIL：tok[2000]（HP −145 / ATK −4 / DEF −2）**

金标准锚点（玩家提供，tok[n]=执行完第 n 步）：

```
tok[1809]: MT15 (7,10)  HP=179 ATK=68 DEF=54  2黄        ← HP 真实恶战 599→179（损 420）
tok[1902]: MT20 (6,10)  HP=591 ATK=68 DEF=54  1蓝2黄
tok[2000]: MT16 (3, 1)  HP=618 ATK=72 DEF=56  无钥匙     ← ATK 68→72、DEF 54→56
```

**排查现状（本次会话 report-only，未改任何代码）**：

- 已排除：漏拾道具（无）、重复战斗/怪物复活（无）、MT12/MT11 地图阻挡（正常）、路径到 tok[1809] 一致。
- **ATK/DEF −4/−2 成因**：成长分两段——1603→1809 (+4/+2)、1809→2000 (+4/+2)。sim 第一段已正确拿到（tok[1809] sim ATK/DEF=68/54 与真值一致）；漏的是第二段，因为 sim 到不了 MT20（约从 tok[1875] 起卡在 MT18↔MT19 反复弹跳），第二段 +4/+2 在 MT20→MT16 段触发，故漏掉。**ATK/DEF 不是独立 bug，是 HP 偏差导致路线走偏的下游表现。**
- **HP 偏差关键发现**：tok[1731] 的 `FLOOR:MT17` fly 是**固定落点 (5,11)**（目标层 downFloor 字段，与飞行前坐标无关），会把 MT11/MT12 段的位置偏移 mask 掉。所以偏差藏在 fly 之前的 MT11/MT12 段（tok[1624-1730]），**HP 差约 206**（sim 进 MT17 时 HP=581 vs 真实 787；两者在 MT17 内同损 608 → sim 581−608=−27、真实 787−608=179，与 tok[1809] 锚点吻合）。
- 旁证：sim 在该段结束时钥匙耗尽（0黄0蓝）vs 真实 2黄1蓝，与"sim 走了少拿钥匙/血瓶的不同路线"一致。
- zombieKnight −396（tok[1744]）经 oracle 核对是**引擎正确值**（此时英雄无 cross，cross 由 MT19 的门事件给予，尚未到达），**非 bug**。

**待办（下个会话）**：需要玩家提供 MT11/MT12 段（tok[1624-1730] 之间）的中间 HP 真值，才能精确定位 206 偏差落在哪一步。因 FLOOR:MT17 fly 固定落点会 mask 位置分叉，仅靠 sim 输出无法判定是"走了不同路线少回血"还是"某怪数据偏高多损"——按铁律不猜测，等真值锚点。

复现脚本：`diag_1603_2000.py`（逐 token 打印 tok[1603..2000] + 3 锚点对比）。
