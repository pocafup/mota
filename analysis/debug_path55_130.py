"""
追踪 decoded[55..130]：找出英雄从 MT1(5,3) 出发后的实际路径，
重点看：是否到达 row1、是否到达 (1,1) 楼梯。
"""
import json, sys
...
def block_reason(state, direction):
    ...
    if e in fl._tile_to_enemy: return ""  # fight = passable
...
def main():
    ...
    print(f"[54] 后: {state.current_floor}({state.hero.x},{state.hero.y}) keys={dict(state.hero.keys)} HP={state.hero.hp}")
    print()

    prev_pos = (state.hero.x, state.hero.y)
    prev_floor = state.current_floor

    for i in range(55, 135):
        tok = tokens[i]
        ...
        # 只打印移动了的、被阻塞的、楼层变化的、或特殊token
        if moved or blk or floor_str or tok.startswith("CHOICE") or tok.startswith("FLOOR"):
            keys_str = f" keys={dict(state.hero.keys)}" if state.hero.keys else ""
            print(f"[{i:3d}] {tok!r:12s} {pf}({px},{py})→({state.hero.x},{state.hero.y}){blk}{floor_str}{intercept}{keys_str}")
        else:
            print(f"[{i:3d}] {tok!r:12s} {pf}({px},{py}) no-op")

    print()
    print(f"[134] 后: {state.current_floor}({state.hero.x},{state.hero.y}) keys={dict(state.hero.keys)} HP={state.hero.hp}")
