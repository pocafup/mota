"""
decoded[0..54] の全トークンと hero 位置を表示。
MT1 の入口からどこを歩くかを完全トレース。
"""
import json, sys
...
def make_state():
    ...
    print(f"init: x={hi['loc']['x']} y={hi['loc']['y']} hp={hi['hp']} atk={hi['atk']} def={hi['def']}")
    print(f"keys={hi.get('keys',{})} items={hi.get('items',{})} flags={hi.get('flags',{})}")
...
def main():
    ...
    print(f"\ntokens[0:10]  = {tokens[0:10]}")
    print(f"tokens[10:20] = {tokens[10:20]}")
    print(f"tokens[20:30] = {tokens[20:30]}")
    print(f"tokens[30:40] = {tokens[30:40]}")
    print(f"tokens[40:55] = {tokens[40:55]}")
    print()
    ...
    for i in range(55):
        ...
        print(f"[{i:2d}] {tok!r:12s} ({px},{py})→({state.hero.x},{state.hero.y}){blk}{floor_str}{keys_str}{hp_str}{tag}")

    print()
    print(f"[54] 後: {state.current_floor}({state.hero.x},{state.hero.y}) HP={state.hero.hp} keys={dict(state.hero.keys)}")
