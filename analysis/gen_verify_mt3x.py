"""扩层第三批 MT29-MT33 提取+逐格校验+连通报告。

复用 gen_verify_mt2x 的 gen_one/report/build_entities 逻辑，仅把数据源换成
extract/mt29_33_raw_combined.json（live engine Playwright dump，double-encoded）。

用法:
    python gen_verify_mt3x.py            # 默认 29 30 31 32 33
    python gen_verify_mt3x.py 29 30      # 只处理指定层
...
COMBINED = Path(__file__).parent / "mt29_33_raw_combined.json"
...
def main():
    args = sys.argv[1:] or ["29", "30", "31", "32", "33"]
