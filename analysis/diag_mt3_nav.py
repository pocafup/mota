"""逐格打印 token[80..210]，重点看 MT3 上半段完整轨迹（含 wall hit）。"""
import json, sys
...
tokens = parse_rle_route(decompress(outer['route']))[:300]
...
print(f"token[79] 结束: floor={state.current_floor} pos=({state.hero.x},{state.hero.y}) "
...
for idx in range(80, 215):
    ...
    print(f"{idx:>4}  {tok:<12}  {fl_after:<4}  "
          ...
          f"{hp_after:>5}  {state.hero.atk:>3}  {yk:>2}  {note}")
