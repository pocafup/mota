"""追踪 token[200..310] 内所有 HP 变化，找 -122 HP 差的根源。"""
import json, sys
...
print(f"token[199] end: floor={state.current_floor} pos=({state.hero.x},{state.hero.y}) "
      f"hp={state.hero.hp} atk={state.hero.atk} def={state.hero.def_}")
print()
print(f"{'idx':>4}  {'tok':<12}  {'fl':<6}  {'from':<10}  {'to':<10}  {'hp':>5}  note")
print('-'*80)

for idx in range(200, 310):
    tok = tokens[idx]
...
print()
print(f"Final: floor={state.current_floor} pos=({state.hero.x},{state.hero.y}) "
      f"hp={state.hero.hp} atk={state.hero.atk} def={state.hero.def_}")
print(f"Truth@300: floor=MT5 hp=604 atk=21 def=10")
