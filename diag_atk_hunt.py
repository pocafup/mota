"""追踪 token[200..300]，找 ATK 从 20→21 的那一步，并打印 MT4 row10 实体。"""
import json, sys
...
print(f"token[199] 结束: floor={state.current_floor} pos=({state.hero.x},{state.hero.y}) "
      f"hp={state.hero.hp} atk={state.hero.atk}")

# 打印 MT4 row10 的实体状态
if 'MT4' in state.floors:
    mt4 = state.floors['MT4']
    print(f"\nMT4 第10行（y=10）entities: {mt4.entities[10]}")
    print(f"MT4 第10行（y=10）terrain:  {mt4.terrain[10]}")
    print(f"MT4 (7,10) entity={mt4.entities[10][7]}  terrain={mt4.terrain[10][7]}")
    print(f"MT4 (8,10) entity={mt4.entities[10][8]}  terrain={mt4.terrain[10][8]}")
    print(f"MT4 (9,10) entity={mt4.entities[10][9]}  terrain={mt4.terrain[10][9]}")

print(f"\n{'idx':>4}  {'tok':<10}  {'fl':<5}  {'from':<10}  {'to':<10}  {'hp':>5}  {'atk':>3}  note")
print('-'*80)
...
print(f"\ntoken[299] 结束: floor={state.current_floor} pos=({state.hero.x},{state.hero.y}) "
      f"hp={state.hero.hp} atk={state.hero.atk} def={state.hero.def_}")
print(f"真值(300): floor=MT5 hp=604 atk=21 def=10")
